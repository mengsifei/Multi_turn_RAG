...
def prompt_strict_extractive_debug(question: str, context: str) -> str:
    # 你原来的 strict（保留，方便对照）
    return f"""<Question>:
{question}
</Question>
...
def prompt_official_grounded_v1(question: str, context: str) -> str:
    return f"""Given the following documents and a user question, generate a concise answer
...
def main(args: argparse.Namespace) -> None:
    ...
    with open(args.taska_file, "r", encoding="utf-8") as fin, \
        ...
        fieldnames = [
            "task_id", "conversation_id", "Collection", "domain",
            "prompt_strict", "prompt_official", "prompt_evidence"
        ]
        ...
        for line in tqdm(fin, desc="Building prompts_multi.csv"):
            n_total += 1
            line = line.strip()
            if not line:
                n_skip_bad += 1
            ...
            row = {
                ...
                "domain": domain,
                "prompt_strict": prompt_strict_extractive_debug(question, context),
                "prompt_official": prompt_official_grounded_v1(question, context),
                "prompt_evidence": prompt_grounded_with_evidence_v1(question, context),
            }
            if args.add_ctx_count:
    ...
    print("========== PROMPTS MULTI CSV SUMMARY ==========")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--task_name", type=str, default="rewrite_gpt",
                    help="rewrite file suffix: <domain>_<task_name>.jsonl")
    ...
    ap.add_argument("--out_csv", required=True, help="output csv with 3 prompt columns")
    ...
    ap.add_argument("--doc_id_in_header", action="store_true")
    ap.add_argument("--fallback_to_last_user", action="store_true", default=True)
    ap.add_argument("--add_ctx_count", action="store_true")
    ap.add_argument("--add_question_source", action="store_true")
    args = ap.parse_args()
    main(args)
