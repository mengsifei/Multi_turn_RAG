...
def ensure_deployment_started(es: ESClient, model_id: str, deployment_id: str, number_of_allocations: int = 1):
    ...
    try:
        es.post(
            ...
            params={"wait_for": "started", "timeout": "30m"},
            body_obj=body,
            ok_codes=(200,),
            retry=2,
            retry_sleep_s=5.0,
        ...
        # 如果已经 started，可能会报 400/409；后面我们会用 stats 验证
        print("[DEPLOY] start failed (will verify via stats):", str(e)[:250])
...
def bulk_index(
    ...
    def is_resume_conflict(status: int, err_type: str, err_reason: str) -> bool:
        if status != 409:
            return False
        # ES 不同版本/不同 op 可能给不同 type，这两个都常见
        if err_type in ("version_conflict_engine_exception", "document_already_exists_exception"):
            return True
        # 兜底：少数情况下 type 不稳定，但 reason 会很明确
        rr = (err_reason or "").lower()
        return ("document already exists" in rr) or ("version conflict" in rr)
    ...
    def flush():
        ...
        for it in items:
            ...
            err_reason = (err.get("reason") or "")

            if 200 <= status < 300:
                n_ok += 1
            elif resume and is_resume_conflict(status, err_type, err_reason):
                n_conflict += 1
            else:
                n_fail += 1
        ...
        if n_fail > 0:
            raise RuntimeError(f"[BULK] {n_fail}/{len(items)} items failed; sample={bad_samples}")
...
def elser_search(es: ESClient, index_name: str, model_id: str, query_text: str, top_k: int) -> List[dict]:
    ...
    r = es.post(f"/{urllib.parse.quote(index_name)}/_search", body_obj=body, ok_codes=(200,))
...
def main():
    ...
    ensure_deployment_started(es, args.model_id, args.deployment_id, number_of_allocations=max(1, args.allocations or 1))
    if args.allocations:
