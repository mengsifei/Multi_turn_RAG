...
def _cache_key(model_tag: str, collection_tag: str, max_len: int, compression_ratio: float, corpus_path: str) -> str:
    st = os.stat(corpus_path)
    sig = f"{model_tag}|{collection_tag}|len={max_len}|cr={compression_ratio}|size={st.st_size}|mtime={int(st.st_mtime)}"
    h = hashlib.sha1(sig.encode("utf-8")).hexdigest()[:16]
    return f"{_safe_tag(collection_tag)}__{_safe_tag(model_tag)}__len{max_len}__cr{compression_ratio:.4f}__{h}"
...
def get_or_build_doc_embeddings(
    *,
    ...
    key = _cache_key(model_tag, collection_tag, max_len, compression_ratio, corpus_path)
...
def load_docsem_corpus_gz(path: str):
    """
    Load doc-semchunk corpus:
      {id, parent_id, start, end, title, text}
...
def _parse_parent_start_end(pid: str):
    parts = pid.split("-")
    if len(parts) < 3:
        return None
    a, b = parts[-2], parts[-1]
...
#     """
#     Accept any of:
#       {"passage_id": "..."} or {"id": "..."} or {"_id": "..."}
#     Ignore base_id/parent_id fields in file, compute parent from passage_id.
#     Return dict: parent_id -> (starts[], ends[], pids[])
#     """
#     by_base = {}
#     with gzip.open(path, "rt", encoding="utf-8") as f:
#         for line in f:
#             o = json.loads(line)
#             pid = o.get("passage_id") or o.get("id") or o.get("_id")
#             if pid is None:
#                 continue
#             pid = str(pid)
#             parsed = _parse_parent_start_end(pid)
#             if parsed is None:
#                 continue
#             base, s, e = parsed
...
def batched_topk_stream(q_emb_cpu: torch.Tensor, doc_emb_cpu: torch.Tensor, k: int, chunk_size: int, device: str):
    ...
    for start in range(0, N, chunk_size):
        ...
        sims = q @ chunk.T
        vals, idx = torch.topk(sims, max(k, sims.shape[0]), dim=1)
        idx = idx + start
...
def aggregate_chunks_to_passages(
    chunk_hits: List[Tuple[str, float]],  # (chunk_id, score)
    chunk_meta: Dict[str, Tuple[str,int,int]],  # chunk_id -> (base_id, start, end)
    span_index: Dict[str, Tuple[List[int], List[int], List[str]]],
    agg: str = "max",
):
    """
    Returns dict passage_id -> score
    agg:
      - "max": passage score = max(chunk_score over overlapping chunks)
      - "max_overlap": passage score = max(chunk_score * overlap_ratio)
    """
    out: Dict[str, float] = {}
...
def main():
    ...
    ap.add_argument("--docsem_corpus_dir", type=str, default="corpora/chunk_level_docsem512_100")
    ...
    ap.add_argument("--agg", type=str, default="max", choices=["max", "max_overlap"])
    ...
    print("top_k_chunks:", args.top_k_chunks, "| top_k_passages:", args.top_k_passages, "| agg:", args.agg)

    for name, cfg in COLLECTIONS.items():
        print(f"\n========== Collection: {name} ==========")

        docsem_path = os.path.join(args.docsem_corpus_dir, f"{name}_docsem512_100.jsonl.gz")
        ...
        # chunk meta for aggregation
        chunk_meta = {cid: (parents[i], starts[i], ends[i]) for i, cid in enumerate(chunk_ids)}

        # encode docs (title + text like baseline)
        doc_texts = [(titles[i] + " " + chunk_texts[i]).strip() for i in range(len(chunk_ids))]
        collection_tag = cfg["collection_name"] + "__docsem512_100"
        ...
        # aggregate to official passage ids
